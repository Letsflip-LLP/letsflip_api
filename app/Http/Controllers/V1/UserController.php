...
class UserController extends Controller
{
    ...
    private function androidVlidateSubscription($request)
    {
        ...
        try {
            ...
            if ((int) $validate_token_purchase['startTimeMillis'] < strtotime(date('Y-m-d H:i:s')) && $product_id == 'private_account') return (new ResponseTransformer)->toJson(400, __('messages.401'), "Expired");
    ...
    public function reportActionUser(Request $request)
    {

        DB::beginTransaction();
        try {

            $model1 = new UserReportModel;
            $model2 = new UserReportModel;
            $model1 = $model1->where('user_id_from', $this->user_login->id);

            if ($request->filled("user_id")) {
                $model1 = $model1->where('user_id_to', $request->user_id);
                $model2->user_id_to = $request->user_id;
            }

            if ($model1->first() == null) {
                $model2->id      = Uuid::uuid4();
                $model2->user_id_from = $this->user_login->id;
