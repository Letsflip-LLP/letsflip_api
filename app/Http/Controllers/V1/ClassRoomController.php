    ...
    public function deleteClassRoom(Request $request)
    {
        ...
        try {
            ...
            if ($class_room == null) {
                ...
                $class_room->Mission()->each(function ($q1) {
                    $q1->MissionContent()->each(function ($q2) {
                        $q2->delete();
                    });
                    $q1->Comment()->each(function ($q2) {
                        $q2->Like()->each(function ($q3) {
                            $q3->delete();
                        });

                        $q2->delete();
                    });
                    $q1->Report()->each(function ($q2) {
                        $q2->delete();
                    });
                    $q1->Respone()->each(function ($q2) {
                        $q2->ResponseContent()->each(function ($q3) {
                            $q3->delete();
                        });
                        $q2->Like()->each(function ($q3) {
                            $q3->delete();
                        });
                        $q2->Comment()->each(function ($q3) {
                            $q3->Like()->each(function ($q4) {
                                $q4->delete();
                            });

                            $q3->delete();
                        });
                        $q2->Report()->each(function ($q3) {
                            $q3->delete();
                        });
                        $q2->Tags()->each(function ($q3) {
                            $q3->delete();
                        });
                        $q2->GradeOverview()->each(function ($q3) {
                            $q3->delete();
                        });
                        $q2->OwnerPoints($this->user_login->id)->each(function($q3){
                            $q3->delete();
                        });

                        $q2->delete();
                    });
                    $q1->Tags()->each(function ($q2) {
                        $q2->delete();
                    });
                    $q1->QuickScores()->each(function ($q2) {
                        $q2->delete();
                    });
                    $q1->ActiveTimer()->each(function ($q2) {
                        $q2->delete();
                    });
                    $q1->Like()->each(function ($q2) {
                        $q2->delete();
                    });
                    $q1->OwnerPoints($this->user_login->id)->each(function ($q2){
                        $q2->delete();
                    });
                ...
                $class_room->Like()->each(function ($q1) {
                    $q1->delete();
                });
                $class_room->PremiumUserAccess()->each(function ($q1) use ($class_room) {
                    $notif_delete_classroom = NotificationManager::addNewNotification(null, $q1->user_id, [
                        "classroom_id" => $class_room->id,
                    ], 26);

                    $q1->delete();
                });
                $class_room->Report()->each(function ($q1) {
                    $q1->delete();
                });
